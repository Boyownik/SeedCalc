...
def list_seeds(tier, seeds, ign):  # List seeds
    ...
    if tier == 1:  # List only T1 seeds
        file.write(f"```css\nWTS Seeds! All T{tier}! & iLvl76+!\n")
        file.write("All prices are in Chaos! You can pay by Exalts or Chaos!\n")
        file.write("\n")
        for s in seeds:
            if s.tier == tier:
                file.write(f"# {s.n} Amount: [{s.amount}] price/seed: [{s.pc}] price/all: [{math.ceil(s.pc * s.amount)}]\n")

        file.write(f"\nWild total amount: [{wild_total}] price/all: [{math.ceil(wildp_total)}]\n")
        file.write(f"Vivid total amount: [{vivid_total}] price/all: [{math.ceil(vividp_total)}]\n")
        file.write(f"Primal total amount: [{primal_total}] price/all: [{math.ceil(primalp_total)}]\n")
