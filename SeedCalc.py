...
def list_seeds(tier, seeds, ign,  ignore):  # List seeds
    wild_total = 0
    vivid_total = 0
    primal_total = 0
    wildp_total = 0.0
    ...
    if tier == 1:  # List only T1 seeds
        ...
        for s in seeds:
            if s.tier == tier:
                if ignore:
                    file.write(f">{s.n} ({s.amount}) pc/e: {s.pc} pc/all: {math.ceil(s.pc * s.amount)}\n")
                else:
...
layout = [
            ...
            [sg.Text("Type in index of your stash to get data of (1,2,3...)"),
             sg.InputText(k="INDEX", default_text=tab_index)],
            [sg.Button("Save")],
